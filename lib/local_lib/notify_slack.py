...
def interval_check(interval_min):
    return local_lib.footprint.elapsed(ERROR_NOTIFY_FOOTPRINT) > interval_min * 60


def interval_clear():
    local_lib.footprint.clear(ERROR_NOTIFY_FOOTPRINT)
...
def error(
    ...
    local_lib.footprint.update(ERROR_NOTIFY_FOOTPRINT)
